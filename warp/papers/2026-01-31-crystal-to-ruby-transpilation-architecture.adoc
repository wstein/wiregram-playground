...
This document provides a comprehensive architecture analysis and recommendations for extending the Warp transpiler to support **bidirectional transpilation**, with specific emphasis on the **Crystal → Ruby** direction. Building on the existing Ruby → Crystal pipeline (documented in `2026-01-31-architecture-analysis-recommendations.adoc`), this analysis addresses the unique challenges, architectural patterns, and implementation strategies required to support reverse transpilation while maintaining lossless round-trip fidelity.
...
|Ruby CST Parser |✓ Complete |Green/Red pattern, lossless preservation
...
|CLI Tooling |✗ Missing |No `warp` CLI yet (planned)
...
1. `2026-01-31-architecture-analysis-recommendations.adoc` - Ruby → Crystal architecture
2. `crystal-ruby-transpiler-proposal.adoc` - Original bidirectional proposal
3. `cst-transpiler-design.adoc` - CST architecture details
4. `ruby-crystal-language-pipeline.adoc` - Unified pipeline design
